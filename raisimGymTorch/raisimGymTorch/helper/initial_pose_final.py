...
def get_hand_rot(vec,  vec_in_hand=[1.2,1,0]):
    ...
    vec_in_hand = vec_in_hand[np.newaxis] # current x-asix direction vector

    vec = vec/np.linalg.norm(vec,axis=-1,keepdims=True) # current random sampling direction vector
    axis = np.cross(vec_in_hand,vec)
    axis = axis/np.linalg.norm(axis,axis=-1,keepdims=True) # ab平面法向量
    angle = np.arccos((vec_in_hand*vec).sum(-1)) # ab向量夹角
    return axis*angle[:,np.newaxis]

# generate initial pose for mano (trianing)
def get_initial_pose_two_hands_gen(obj_mesh, non_aff_mesh, easy=False, hand_flag:str="right", x:float=0.0,y:float=0.0,z:float=0.0):
    ...
    dis_to_target = np.random.uniform(0.30, 0.40)
    ...
    while not got_sample:
        ...
        if sample_cnt > 50:
            # print("sample too lot !!!!")
            return None, None, None, None

        current_direction = np.random.uniform(-1, 1, (1, 3))
        if abs(x) > 0.001 or abs(y) > 0.001 or abs(z) > 0.001:
            current_direction[0][0] = x #+ np.random.uniform(-0.1, 0.1)
            current_direction[0][1] = y #+ np.random.uniform(-0.1, 0.1)
            current_direction[0][2] = z #+ np.random.uniform(-0.1, 0.1)
        ...
        if non_aff_mesh is not None:
            ...
            if np.linalg.norm(locations - aff_center) > min_dis_non_af:
                #print('got stuck because of sample point in non aff mesh')
                continue

        target = locations + 0.01 * (aff_center - locations)/np.linalg.norm(aff_center - locations)
        # target = locations
        pos = target + dis_to_target * dir
        ...
        if non_aff_mesh is not None:
            ...
            if np.linalg.norm(hand_center_pos - aff_center) > np.linalg.norm(hand_center_pos - non_aff_center):
                # print(obj_name)
                # print('got stuck because of center distance in non aff mesh')
            ...
            if len(non_aff_locations) > 0 and np.linalg.norm(ray_origins - non_aff_locations) < np.linalg.norm(ray_origins - locations):
                # print(obj_name)
                # print('got stuck because of ray intersection in non aff mesh')
                continue

        got_sample = True
    return rot12, pos, bias, dir
    # return rot12, pos, bias, opt_pos

# generate initial pose for mano (trianing)
def get_initial_pose(obj_mesh, non_aff_mesh, easy=False, hand_flag:str="right"):
    # sample 3000 points from the pytorch3d mesh
    points = obj_mesh.vertices if torch.is_tensor(obj_mesh.vertices) else torch.tensor(obj_mesh.vertices,dtype=torch.float32).unsqueeze(0)
    obj_pcd = points.detach().cpu().numpy()
    aff_center = obj_mesh.centroid
...
def get_initial_pose_top(obj_mesh, non_aff_mesh, easy=False):
    # sample 3000 points from the pytorch3d mesh
    points = obj_mesh.vertices if torch.is_tensor(obj_mesh.vertices) else torch.tensor(obj_mesh.vertices,dtype=torch.float32).unsqueeze(0)
    obj_pcd = points.detach().cpu().numpy()
    aff_center = obj_mesh.centroid
    ...
    while not got_sample:
        ...
        if sample_times < 200:
            current_direction = np.random.uniform(-1, 1, (1, 3))
            current_direction[0][2] = np.abs(current_direction[0][2])
            dir = current_direction / np.linalg.norm(current_direction, axis=1, keepdims=True)
            if np.linalg.norm(dir[:, :2]) > (dir[0][2]):
                continue
            ...
            current_direction = np.random.uniform(-1, 1, (1, 3))
            dir = np.zeros_like(current_direction)
            dir[0][2] = 1
            ...
            infeasible = True
    return rot12, pos, bias, infeasible
    # return rot12, pos, bias, opt_pos

# get the label for mano (evaluation)
def get_initial_pose_label(obj_mesh, non_aff_mesh, easy=False):
    # sample 3000 points from the pytorch3d mesh
    points = obj_mesh.vertices if torch.is_tensor(obj_mesh.vertices) else torch.tensor(obj_mesh.vertices,dtype=torch.float32).unsqueeze(0)
    obj_pcd = points.detach().cpu().numpy()
    aff_center = obj_mesh.centroid
    ...
    while not got_sample:
        ...
        if lat_length > 0.12:
            # print('got stuck because of lat length')
            continue
        ...
        opt_pos = target + (0.12 + long_length) * dir
    # return rot12, pos, bias
    return rot12, pos, bias, opt_pos, dir, axis, rand_offset
...
def get_initial_pose_faive_random(obj_mesh, non_aff_mesh, hand_type='faive', top=False, easy=False):
    # sample 3000 points from the pytorch3d mesh
    points = obj_mesh.vertices if torch.is_tensor(obj_mesh.vertices) else torch.tensor(obj_mesh.vertices,
                                                                                       dtype=torch.float32).unsqueeze(0)
    obj_pcd = points.detach().cpu().numpy()
    ...
    while not got_sample:
        current_direction = np.random.uniform(-1, 1, (1, 3))
        if top:
            current_direction[:, 2] = np.abs(current_direction[:, 2]) * (np.abs(current_direction[:, 2]) > 0.5) + 0.5 * (np.abs(current_direction[:, 2]) <= 0.5)
        dir = current_direction / np.linalg.norm(current_direction, axis=1, keepdims=True)
...
def get_initial_pose_allegro_arm_rand_test(obj_mesh, obj_pcd_ori, x_dir, aff_center, obj_mat, top=False, hand="allegro"):
    points = obj_pcd_ori
...
def sample_rot_mats(hand_dir_x_w, num_samples, visible_points_w):
    # Sample 10 perpendicular unit vectors
    ...
    # Sample 10 vectors evenly distributed around the circle
...
#     if (lat_length > 0.18) and (top is False):
#         return None
#     x_dir = axis.reshape(1,3)
#     x_dir_in_world = np.matmul(obj_mat, x_dir.T).T
#     if x_dir_in_world[0,1] > 0:
#         x_dir = -x_dir
#     y_dir = np.cross(dir, x_dir)
#
#     rot_mat= -np.stack((x_dir, y_dir, dir), axis=-1)
#
#     return rot_mat
...
def get_initial_pose_shadow_easy(obj_mesh, non_aff_mesh, hand_type='faive'):
    ...
    while not got_sample:
        ...
        if lat_length > 0.12 and sample_time < 50:
            continue
        ...
        if rotated_lat_length > 0.12 and sample_time < 50:
            continue
        ...
        line_direction = (locations - aff_center) / np.linalg.norm(locations - aff_center)
        if (line_direction * dir).sum() < 0. and sample_time < 50:
            # print('got stuck because of line direction')
            continue
...
def get_initial_pose_two_hands(obj_mesh, non_aff_mesh):

    while True:
        rot_r, pos_r, bias_r, dir_r = get_initial_pose_two_hands_gen(obj_mesh, non_aff_mesh, easy=False, hand_flag="right")
        if rot_r is not None:
            rot_l, pos_l, bias_l, dir_l = get_initial_pose_two_hands_gen(obj_mesh, non_aff_mesh, easy=False, hand_flag="left", x=-dir_r[0][0],y=-dir_r[0][1],z=-dir_r[0][2])
            if rot_l is not None:
                return rot_r, pos_r, bias_r, dir_r, rot_l, pos_l, bias_l, dir_l 

def get_initial_pose_one_hands(obj_mesh, non_aff_mesh, hand_flag="right"):

    while True:
        rot, pos, bias, dir = get_initial_pose_two_hands_gen(obj_mesh, non_aff_mesh, hand_flag=hand_flag)
        if rot is not None:
            return rot, pos, bias
