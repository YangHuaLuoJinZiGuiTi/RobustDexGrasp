...
//            mano_r_->setState(action_r.cast<double>(), gv_set_r_);
        ...
        void computeGraspCosts() {
            ...
            for (const auto& pair : aggregated_forces) {
                ...
                // std::cerr << "total_force" << -total_force(2) << std::endl;
            }
            ...
            if (num_grasp_contacts_ >= 1) { // Need at least 1 contact to compute force closure
                ...
                Eigen::VectorXd grasp_acceleration_error_norm = Eigen::VectorXd::Zero(6);
                grasp_acceleration_error_norm(2) = -9.81; // z方向的重力
                friction_cone_cost_ = 0.0;
            }
        }
        ...
        Eigen::VectorXd computeGraspAcc(const std::vector<Eigen::Vector3d>& contact_points,
            ...
            if (n_contacts < 1) return F_obj / (obj_weight/9.81 + 1e-8); // 
            ...
            return diff / (obj_weight/9.81 + 1e-8);  
        }
        
        // 计算摩擦锥成本
        // NOTE: 如果一开始的接触点不好，他不就不想更新接触点了嘛？
        /// Contact coefficient = 1 - F_t/ (\mu * F_n)
