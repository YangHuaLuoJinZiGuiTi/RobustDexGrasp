...
obj_ori_list = obj_ori_list[:1] # For Debug
...
for update in range(10):
    ...
    for i in range(num_envs):
        mass_list.append(env.getObjectTotalMass(i, 'object'))
    ...
    if len(success_indices) > 0:
        ...
        f_g = np.average(
            max_force_array.mean(axis=0)[success_indices] / np.array(mass_list)[success_indices] / 10
            )
        ...
        f_max = np.max(max_force_array, axis=0)[success_indices]
        print(f"current F_max  (N) / G_obj (N) for success obj: {f_g} ; F_max_mean: {f_max_mean}N; F_max: {f_max}" )
