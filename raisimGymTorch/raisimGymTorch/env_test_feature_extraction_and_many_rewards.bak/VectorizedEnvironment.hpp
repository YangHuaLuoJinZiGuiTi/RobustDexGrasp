...
#pragma omp parallel for
    ...
    void get_contact_info(std::vector<std::vector<Eigen::Vector3d>>& all_points,
                          std::vector<std::vector<Eigen::Vector3d>>& all_normals,
                          std::vector<std::vector<Eigen::Vector3d>>& all_forces) {
    all_points.resize(num_envs_);
    all_normals.resize(num_envs_);
    all_forces.resize(num_envs_);
    
#pragma omp parallel for
    for (int i = 0; i < num_envs_; i++) {
        environments_[i]->get_contact_info(all_points[i], all_normals[i], all_forces[i]);
    }
}
