...
namespace raisim {
    ...
    public:
        ...
        explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
            ...
            prevLocalContactPositions_.clear();
            ...
            // build mano_r_ index-to-name mapping 
            std::vector<std::string> hand_pose_bodies, arm_pose_bodies;
            mano_r_->getBodies(hand_pose_bodies, false, false);
            mano_r_->getBodies(arm_pose_bodies, true, false);
            for (const auto& name : hand_pose_bodies) {
                size_t idx = mano_r_->getBodyIdx(name);
                bodyIndexToName_[idx] = name;
            }
            for (const auto& name : arm_pose_bodies) {
                size_t idx = mano_r_->getBodyIdx(name);
                bodyIndexToName_[idx] = name;
            }
...
//            mano_r_->setState(action_r.cast<double>(), gv_set_r_);
        ...
        void detectSliding() {
            ...
            std::map<int, raisim::Vec<3>> currentLocalPositions;

            // 1. Calculate current contact points in local coordinates
            for (const auto& contact : contacts) {
                int bodyIdx = contact.getlocalBodyIndex();

                std::cerr << "[bodyIndexToName_]"
          << bodyIndexToName_ << std::endl;
                // Get body name from our mapping
                auto it = bodyIndexToName_.find(bodyIdx);
                if (it == bodyIndexToName_.end()) continue;

                std::string bodyName = it->second;

                // Convert global position to body local coordinates
                raisim::Vec<3> globalPos = contact.getPosition();
                raisim::Vec<3> bodyPos;
                raisim::Mat<3,3> bodyRot;
                mano_r_->getFramePosition(bodyName, bodyPos);
                mano_r_->getFrameOrientation(bodyName, bodyRot);

                // p_local = R^T (p_global - p_body)
                raisim::Vec<3> localPos = bodyRot.transpose() * (globalPos - bodyPos);
                currentLocalPositions[bodyIdx] = localPos;
            }

            // 2. Detect sliding with distance-based penalty scaled by contact force
            sliding_penalty = 0.0;  // Reset for current frame
            total_sliding_speed = 0.0;

                        std::cerr << "[detectSliding] currentLocalPositions_ size="
          << currentLocalPositions.size() << std::endl;

            // Use the contactMapping_r_ index (idx) for accessing per-contact arrays
            for (const auto& [bodyIdx, currLocalPos] : currentLocalPositions) {
                // check that we had this contact in the previous frame
                auto prevIt = prevLocalContactPositions_.find(bodyIdx);
                if (prevIt == prevLocalContactPositions_.end()) {
                    continue;
                }
                const auto& prevLocalPos = prevIt->second;

                // Calculate displacement in local coordinates (distance in meters)
                double slidingDist = (currLocalPos - prevLocalPos).norm();

                // Find the corresponding contact index from our mapping
                auto it = contactMapping_r_.find(bodyIdx);
                if (it == contactMapping_r_.end()) {
                    // no mapping for this body index
                    continue;
                }
                int idx = it->second;

                // safety check on index
                if (idx < 0 || idx >= num_contacts) continue;

                // Get contact force for this contact point (already calculated in updateObservation)
                double contact_force = impulses_r_af[idx] / control_dt_;

                // Calculate penalty: sliding distance * contact force (contact force scaling optional)
                double penalty = 0.0;
                if (slidingDist > SLIDING_DISTANCE_THRESHOLD) {
                    // Penalty scales with sliding distance (multiplying by contact_force is optional)
                    penalty = slidingDist; // * contact_force;
                }

                // Store for visualization/debugging using the mapped index (NOT the iteration order)
                sliding_speeds_[idx] = slidingDist;
                sliding_penalties_[idx] = penalty;

                total_sliding_speed += slidingDist;
                sliding_penalty += penalty;
            }

            // 3. Update previous frame data
            prevLocalContactPositions_ = currentLocalPositions;
        }
        ...
        void updateObservation() {
            ...
            sliding_speeds_.setZero(num_contacts);
            sliding_penalties_.setZero(num_contacts);
            total_sliding_speed = 0.0;
            ...
            std::cerr << "[detectSliding] sizes: contactMapping=" << contactMapping_r_.size()
          << " impulses=" << impulses_r_af.size()
          << " sliding_speeds=" << sliding_speeds_.size()
          << " sliding_penalties=" << sliding_penalties_.size()
          << " prevLocal=" << prevLocalContactPositions_.size()
          << std::endl;

            
            std::cerr << "[detectSliding] prevLocalContactPositions_ size =" <<  prevLocalContactPositions_.size() 
          << " contacts=" << mano_r_->getContacts().size() << std::endl;
            // ========= 
            ...
            // ======== ... 函数尾：
            std::cerr << "[detectSliding] EXIT total_sliding_speed=" << total_sliding_speed 
                    << " sliding_penalty=" << sliding_penalty << std::endl;


            for (const auto &p : contactMapping_r_) {
                std::cerr << "[mapping] key=" << p.first << " -> value=" << p.second << std::endl;
            }

            std::cerr << "[detectSliding] after update prevLocalContactPositions_ size="
          << prevLocalContactPositions_.size() << std::endl;
           ...
           raisim::RotmatToEuler(wrist_mat_r_trans, hand_pose_trans);
            ...
            global_state_ << obj_pose_wrist.e(),
                             ...
                             max_contact_force_;
                             /// For debug
                             prevLocalContactPositions_.size();
        }
    ...
    private:
        ...
        // for sliding
        Eigen::VectorXd sliding_speeds_;
        Eigen::VectorXd sliding_penalties_;
        double total_sliding_speed;
        std::map<int, raisim::Vec<3>> prevLocalContactPositions_;
