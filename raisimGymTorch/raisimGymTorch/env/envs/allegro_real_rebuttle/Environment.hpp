...
//            actionMean_r_.tail(gcDim_-6) = gc_set_r_.tail(gcDim_-6);
        ...
        void final_reset_state(const Eigen::Ref<EigenVec>& init_state_r, bool release_hand, bool sim_flag, const Eigen::Ref<EigenVec>& set_arm, bool no_wait = false) final {
            ...
            mano_r_->setState(pTarget_clipped_r, gv_set_r_, sim_flag, true, no_wait=no_wait);
        }
        ...
        float* step_all(const Eigen::Ref<EigenVec>& action_r, const Eigen::Ref<EigenVec>& action_l, bool sim_flag) {
            ...
            //     }
            // }
            // if (max_step_distance_hand > 0.1) {
            //     hand_delay_cnt = round(max_step_distance_hand / 0.08) + 1.0;
            //     if (hand_delay_cnt > 4.0) {
            //         hand_delay_cnt = 4.0;
            //     }
            //     if (hand_delay_cnt > 1.0) {
        ...
        void updateObservation(bool start = false, bool force_sim = false) {
            mano_r_->updateObservation(start, force_sim);
