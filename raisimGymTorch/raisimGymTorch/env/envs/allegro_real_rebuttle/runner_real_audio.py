...
def update_label(label):
    ...
    label.after(0, lambda: label.config(text=f"\n\n\nwait for init all ............ ", font=("Helvetica", 72), wraplength=1700))
    ...
    while True:
        ...
        if sample_pc_mode == 'sam' or sample_pc_mode == 'manual':
            ...
            env.final_reset_state(np.zeros((num_envs, 22), dtype='float32'), True, False, reset_top, False)
        elif sample_pc_mode == 'audio':
            t1 = time.time()
            pth = "/home/ubuntu/hand/github/vision_dex/raisimGymTorch/raisimGymTorch/env/hardware/realsense/"
            # 先异步拍摄获取点云，同时录入音频。
            rgbd.async_save_rgbd(pth)

            label.after(0, lambda: label.config(text=f"\n\n\nplease input the audio ............ ", font=("Helvetica", 72), wraplength=1700))
            ...
            label.after(0, lambda: label.config(text=f"\n\n\nwill take action....", font=("Helvetica", 72), wraplength=1700))
            mask = sam.calculate_mask(rgb_frame, input_point, input_label, input_box)
            t5 = time.time()
            print(f"-------------t45 = {t5 - t4}")
        ...
        label.after(0, lambda: label.config(text=f"\n\n\n grasping ............ ", font=("Helvetica", 72), wraplength=1700))
        for i in range(num_envs):
        ...
        for sim_flag in [False]: # True, False
            ...
            for i in range(50):
                if env.get_no_wait_finish_flag() is False:
                    time.sleep(0.2)
                else:
                    time.sleep(0.75)
                    break
            ...
            print("will release ..... ")
            env.final_reset_state(action_r, True, sim_flag, lift_topplace, False)
            print("finsh all")
...
def main():
    ...
    # Start a new thread to update the label
    thread = threading.Thread(target=update_label, args=(label,))
    thread.daemon = True  # Daemonize the thread to exit when main program exits
    thread.start()
