...
# weight_saved = '2024-01-30-16-35-15/full_7300_r.pt'
...
# weight_path_student = '2024-06-21-10-52-17/full_700_r.pt'
weight_path_student = '2024-06-21-14-45-13/full_37000_r.pt'
...
# weight_path_student_new = '2024-06-21-12-51-00/full_0_r.pt'
# # actor_student_r_new = ppo_module.Actor(
# #     ppo_module.MLP(cfg['architecture']['policy_net'], activations, ob_dim_r, act_dim),
# #     ppo_module.MultivariateGaussianDiagonalCovariance(act_dim, num_envs, 1.0, NormalSampler(act_dim)), device)
# # prop_latent_encoder_new = ppo_module.LSTM_StateHistoryEncoder(tobeEncode_dim, prop_latent_dim, t_steps, device)
# checkpoint_student_new = torch.load(saver.data_dir.split('eval')[0] + weight_path_student_new, map_location=torch.device('cpu'))
# # actor_student_r_new.architecture.load_state_dict(checkpoint_student['actor_architecture_state_dict'])
# # actor_student_r_new.distribution.load_state_dict(checkpoint_student['actor_distribution_state_dict'])
# # prop_latent_encoder_new.load_state_dict(checkpoint_student['prop_latent_encoder_state_dict'])
#
# print(str(checkpoint_student['actor_architecture_state_dict']) == str(checkpoint_student_new['actor_architecture_state_dict']))
# print(str(checkpoint_student['actor_distribution_state_dict']) == str(checkpoint_student_new['actor_distribution_state_dict']))
# print(str(checkpoint_student['prop_latent_encoder_state_dict']) == str(checkpoint_student_new['prop_latent_encoder_state_dict']))
...
for update in range(args.num_iterations):
    ...
    for step in range(n_steps_r):
        ...
        student_mlp_obs = torch.cat((torch.from_numpy(obs_r[:, -ob_dim_r:-ob_dim_r+tobeEncode_dim]), student_latent.cpu(), torch.from_numpy(obs_r[:, -ob_dim_r+tobeEncode_dim+prop_latent_dim:])), dim=1).to(device)
