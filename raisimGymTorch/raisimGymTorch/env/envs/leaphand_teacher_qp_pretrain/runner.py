path_to_ensure = '/home/ubuntu/hand/github/vision_dex/raisimGymTorch'
import sys
if path_to_ensure not in sys.path:
    sys.path.insert(0, path_to_ensure)
else:
    # 如果已经在，将其移到前面
    sys.path.remove(path_to_ensure)
    sys.path.insert(0, path_to_ensure)

import numpy
from ruamel.yaml import YAML, dump, RoundTripDumper
from raisimGymTorch.env.bin import leaphand_teacher_qp_pretrain as mano
from raisimGymTorch.env.RaisimGymVecEnvOther import RaisimGymVecEnvTest as VecEnv
from raisimGymTorch.helper.raisim_gym_helper import ConfigurationSaver, load_param, tensorboard_launcher
from raisimGymTorch.env.bin.leaphand_teacher_qp_pretrain import NormalSampler
...
parser.add_argument('-qp', '--use_qp_reward', default=False)

args = parser.parse_args()
weight_path = args.weight
cfg_grasp = args.cfg
use_QP_reward = args.use_qp_reward
