...
def allegro_to_urdf(joints):
    joints = np.array(joints)
    ret_joints = np.zeros(16)
    idx = [0,4,8]
    idx_1 = [1,5,9]
    ret_joints = joints + 3.14159
    ret_joints[idx] -= 1.5708
    tmp = ret_joints[idx]
    ret_joints[idx] = ret_joints[idx_1]
    ret_joints[idx_1] = tmp
    return ret_joints
def urdf_to_allegro(joints):
    joints = np.array(joints)
    ret_joints = np.zeros(16)
    idx = [0,4,8]
    idx_1 = [1,5,9]
    ret_joints = joints - 3.14159
    ret_joints[idx_1] += 1.5708
    tmp = ret_joints[idx]
    ret_joints[idx] = ret_joints[idx_1]
    ret_joints[idx_1] = tmp
    return ret_joints
