...
public:
    ...
    void set_log_data(const Eigen::VectorXd &save_target, const Eigen::VectorXd &save_current) {
        for (int i = 0; i < platform_gc_dim_; i++) {
            save_target_[i] = save_target[i];
            save_current_[i] = save_current[i];
        }
    }
    /**
    ...
    void setPdTargetArmBaseFrameAutoIK(const Eigen::VectorXd &posTarget, const Eigen::VectorXd &velTarget) {
        if (flying_hand_mode_) {
            setPdTarget(posTarget, velTarget);
            return;
        }
     ...
     * @return None
     */
    void updateObservation(bool start = false, bool force_sim = false) {
        if (force_sim) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                pinocchio_joint[14 + i] = gc[10 + i];
                pinocchio_joint[18 + i] = gc[14 + i];
            }
     ...
     * @return None
     */
    void getState(Eigen::VectorXd &genco, Eigen::VectorXd &genvel, bool force_sim = false) {
        if (force_sim) {
            arm_hand_platform_->getState(genco, genvel);
            return;
        }
    ...
    void setStateArmBaseFrameAutoIK(const Eigen::VectorXd &genco, const Eigen::VectorXd &genvel) {
        if (flying_hand_mode_) {
            setState(genco, genvel);
            return;
        }

        std::cout << "pinocchio start eef :::::: " << genco.head(6).transpose() << std::endl;

        Eigen::VectorXd new_target = genco;
        Eigen::VectorXd get_q(arm_dim_);
        HardwareKinematic::IK_ERRCODE ret = getIKSolve(genco.head(6), get_q);
        
        std::cout << "pinocchio ik result :::::: " << get_q.transpose() << std::endl;
