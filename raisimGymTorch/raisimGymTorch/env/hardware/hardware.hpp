...
public:
    ...
    /**
     * same as `setPdTarget` but solve IK automaticly in simulation world Frame
     * @param[in] posTarget first is eef pose then hand joint position which are generated by policy in world frame
     * @param[in] velTarget may be zero
     * @return None
     */
    void setPdTargetWorldFrameAutoIK(const Eigen::VectorXd &posTarget, const Eigen::VectorXd &velTarget) {
        if (flying_hand_mode_) {
            setPdTarget(posTarget, velTarget);
            return;
        }
     ...
     * @return None
     */
    void getBasePose(const Eigen::VectorXd &user_input, const bool flying_hand_mode, raisim::Vec<3> &pos, raisim::Mat<3,3> &ori) {
    ...
    // raisim API only used in hand
    int getNumContacts() {
        return hand_->getNumContacts();
    }
    int getNumBodies() {
        return hand_->getNumBodies();
    }
    int getNumFinger() {
        return hand_->getNumFinger();
    }
    void getArmBodyParts(std::vector<std::string> & get_vec) {
        arm_->getBodyParts(get_vec);
    }
    void getHandBodyParts(std::vector<std::string> & get_vec) {
        hand_->getBodyParts(get_vec);
    }
    void getArmContactBodies(std::vector<std::string> & get_vec) {
        arm_->getContactBodies(get_vec);
    }
    void getHandContactBodies(std::vector<std::string> & get_vec) {
        hand_->getContactBodies(get_vec);
    }
    ...
    // raisim API only used in platform
