...
def train(main_cfg: DictConfig):
    ...
    hand = importlib.import_module(f"raisimGymTorch.env.bin.{main_cfg.task_name}")
    NormalSampler = hand.NormalSampler
    ...
    if main_cfg.exp.log_name is not None:
        wandb.init(project=exp_name, config=cfg, name=main_cfg.exp.log_name)
    ...
    for update in range(main_cfg.exp.num_iterations + 1):
        ...
        if update % cfg['environment']['eval_every_n'] == 0 and main_cfg.exp.log_name is not None:
        ...
        if main_cfg.exp.log_name is not None and not is_evaluation:  # 只在非评估阶段记录训练奖励
