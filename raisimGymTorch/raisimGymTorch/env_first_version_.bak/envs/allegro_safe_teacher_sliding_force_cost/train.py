...
from raisimGymTorch.env.bin import allegro_safe_teacher_sliding_force_cost as hand
from raisimGymTorch.env.RaisimGymVecEnvOther import RaisimGymVecEnvTest as VecEnv
from raisimGymTorch.helper.raisim_gym_helper import ConfigurationSaver, load_param_p3o
from raisimGymTorch.env.bin.allegro_safe_teacher_sliding_force_cost import NormalSampler
...
parser.add_argument('-itr', '--num_iterations', type=int, default=20000)
parser.add_argument('-re', '--load_trained_policy', action="store_true") # default False 
...
for update in range(args.num_iterations):
    ...
    for i in range(num_envs):
        ...
        cost_r_sum.append({})
        for idx, cost_name in enumerate(cfg['environment']['p3o']['cost_limits'].keys()):
            cost_r_sum[i][cost_name] = 0.0
    ...
    for step in range(current_steps):
        ...
        # for debug sliding:
        # print(">>> contact prevLocations' size: ", global_state[:, -1])
        ...
        for i in range(num_envs):
            ...
            # cost
            for idx, cost_name in enumerate(cfg['environment']['p3o']['cost_limits'].keys()):
                cost_r_sum[i][cost_name] += cost_r[i][idx]
    ...
    ave_cost = {}
    for cost_name in cfg['environment']['p3o']['cost_limits'].keys():
        ave_cost[cost_name] = 0.0


    for i in range(len(cost_r_sum)):
        for cost_name in ave_cost.keys():
            ave_cost[cost_name] += cost_r_sum[i][cost_name]

    for cost_name in ave_cost.keys():
        ave_cost[cost_name] = ave_cost[cost_name] / (len(cost_r_sum) * n_steps_r)
    ...
    if args.log_name is not None and not is_evaluation:  # 只在非评估阶段记录训练奖励
        ...
        wandb.log(ave_cost, step=update)   
        

    if args.log_name is None:
        print(ave_reward, file=sys.stdout)
        print(ave_cost, file=sys.stdout)
    ...
    for cost_name, value in ave_cost.items():
        print(f"average {cost_name}: {value:.10f}", file=sys.stdout)
    print('{:<40} {:>6}'.format("time elapsed in this iteration: ", '{:6.4f}'.format(end - start)), file=sys.stdout)
    print('{:<40} {:>6}'.format("time to finish: ", '{:6.4f} h'.format( (end - start) * (args.num_iterations - update) / 3600
                                                                     )), file=sys.stdout)
