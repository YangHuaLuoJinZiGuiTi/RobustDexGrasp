...
class RaisimGymVecEnvTest:
    ...
    def observe(self, contain_non_aff, allegro=False):
        ...
        # print("dis_info shape", dis_info.shape)
        ...
        # r_obj_new = np.zeros((num_envs, 3), 'float32')
        # r_obj_new = R.from_euler('XYZ', r_obj_new, degrees=False)
        # show_point = r_obj_new.apply(af_points.reshape(-1, 3).cpu()).reshape(num_envs, -1).astype('float32')
